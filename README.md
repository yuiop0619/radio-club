...
当前连接使用同站 Node 服务及 HttpOnly 会话。树洞进页即自动连接公开服务，纸条默认按“公开投递”送出：正文、署名、心情和真人回复会上传并对其他访客可见；把可见范围切回“仅本机”则只留在这台设备（切回后不再被默认值覆盖）。委托档案不会随纸条上传。恢复码可在另一浏览器恢复同一身份，必须私下保存。选择 CloudBase 部署时才加载其 SDK，身份恢复取决于对应登录配置。
